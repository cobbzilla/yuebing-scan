...
const execAnalyze = (assetDir, downloaded, profile, job, logger, sourceAsset, conn, clock, analysisResults) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const response = yield applyProfile(logger, downloaded, profile.media, profile.name, outDir, assetPath(sourceAsset.name), conn, analysisResults);
    if (profile.operationObject.func) {
        ...
        if (response.args && response.args.length > 0) {
            ...
            const result = yield runExternalCommand(profile.operationObject.command, response.args, logger);
