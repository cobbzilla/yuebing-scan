...
const execAnalyze = (assetDir, downloaded, profile, profileJob, logger, sourceAsset, clock) => __awaiter(void 0, void 0, void 0, function* () {
...
export const analyzeAsset = (analyzer, sourceAsset, downloaded, profile) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    yield execAnalyze(assetDir, downloaded, profile, profileJob, analyzer.config.logger, sourceAsset, analyzer.clock);
    const existingAnalysis = yield profileJobRepo.safeFindById(jobName);
    if (existingAnalysis) {
        console.info(`analyze: updating analysis profileJob: ${JSON.stringify(profileJob)} (previous=${JSON.stringify(existingAnalysis)})`);
...
export const analyzeSourceAsset = (analyzer, sourceAsset) => __awaiter(void 0, void 0, void 0, function* () {
    const downloaded = yield downloadSourceAsset(analyzer.config.downloadDir, sourceAsset, analyzer.config.sourceRepo(), analyzer.clock);
    if (!downloaded)
    ...
    for (const analyzeProfile of analysisProfiles) {
        yield analyzeAsset(analyzer, sourceAsset, downloaded, analyzeProfile);
    }
