...
const claimLock = (lock, systemName, clock, lockRepo, targetType, targetId, logger) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b;
    ...
    const verified = yield lockRepo.safeFindFirstBy(targetType.typeName, targetId);
    if ((verified === null || verified === void 0 ? void 0 : verified.owner) === systemName) {
        return verified;
    }
    else {
...
export const acquireLock = (systemName, clock, logger, lockRepo, target, targetType, interval) => __awaiter(void 0, void 0, void 0, function* () {
    var _c, _d;
    const targetId = targetType.id(target);
    const lock = yield lockRepo.safeFindFirstBy(targetType.typeName, targetId);
    if (!lock) {
        const toCreate = {
            owner: systemName,
            status: "started",
        ...
        const found = yield lockRepo.safeFindFirstBy(targetType.typeName, targetId);
        ...
        if (clock.now() - lock.finished > interval) {
            return yield claimLock(lock, systemName, clock, lockRepo, targetType, targetId, logger);
        }
        else {
