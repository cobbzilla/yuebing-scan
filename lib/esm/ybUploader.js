...
import { connectVolume, UploadJobTypeDef } from "yuebing-model";
        ...
        while (!uploader.stopping) {
            ...
            try {
                ...
                if (job) {
                    ...
                    const lock = yield acquireLock(uploader.config.systemName, uploader.clock, uploader.config.logger, uploadJobRepo, UploadJobTypeDef.id(job), UploadJobTypeDef, uploadTimeout(job.size));
                    if (lock) {
