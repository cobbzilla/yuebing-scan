...
export class YbScanner {
    ...
    scanLibrary(lib, timeout) {
        return __awaiter(this, void 0, void 0, function* () {
            let lock = null;
            try {
                lock = yield acquireLock(this.config.systemName, this.clock, this.config.logger, this.config.libraryScanRepo(), lib.name, LibraryScanTypeDef, timeout);
                if (!lock) {
                    this.config.logger.debug(`scanLibrary: system=${this.config.systemName} lib=${lib.name} error=acquiring_lock`);
                    return;
                }
